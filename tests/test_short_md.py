...
def test_rmsd_analysis_selects_only_protein_bb_beads(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tpr = tmp_path / "stage.tpr"
    xtc = tmp_path / "stage.xtc"
    tpr.touch()
    xtc.touch()
    ...
    def fake_run(command: list[str], **kwargs: object) -> CompletedProcess[str]:
        ...
        if command[1] == "select":
            Path(command[command.index("-on") + 1]).write_text("[ Protein_Backbone ]\n1 2 3\n")
        else:
            Path(command[command.index("-o") + 1]).write_text("0.0 0.10\n1.0 0.20\n")
        return CompletedProcess(command, 0, "", "")

    monkeypatch.setattr("streamlit_app.short_md.subprocess.run", fake_run)
    result = run_short_md_analysis("rmsd", tpr, xtc, tmp_path / "analysis", "gmx")

    assert calls[0][0][-1] == 'group "Protein" and name BB'
    assert "-n" in calls[1][0]
    assert calls[1][1] == "0\n0\n"
    assert result.points == [(0.0, 0.10), (1.0, 0.20)]
